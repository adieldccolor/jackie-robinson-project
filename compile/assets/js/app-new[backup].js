var base = window.base_font_size || 10,
	...
	timer = timer || [],
	owl = owl || null, owlSmall = owl || null, owlHistory = owl || null,
	_images = 'bg',
	historyPan = {
		...
		zoom: function(i){
			...
			$('body').on('click', '.trigger-zoom', function(){
				...
				**** OLD HISTORY TIMELINE VERSION ****
				...
				e.stopPropagation(); e.preventDefault();
				$('.search-button, .search-trigger-btn,body').toggleClass('search-open');
			...
			owl.on('changed.owl.carousel', function(event) {
				$('#owl-demo-2 .owl-item').removeClass('active');
				historyPan.getCurrent();
			});
			...
			owlSmall.owlCarousel({
			  ...
			  onInitialized: historyPan.setFirst
			});
			...
			owlSponsors.owlCarousel({
			  ...
			  // itemsMobile : [479,2],
			  autoWidth: true,
			  itemsCustom : [[1199,7], [979,5], [768,4], [579,3], [479,2], [320,1]],
			  responsive: {
			  	0: {
			  		items: 1
			  	}, 320: {
			...
			owlSponsors = $('#owl-sponsor').data('owlCarousel');

			$('#owl-demo-2 .owl-item a').on('click', function(e){
				e.stopPropagation(); e.preventDefault();
				var current = $(this).closest('.owl-item').index();
				console.log(current);
				// $(this).closest('.owl-item').addClass('active').siblings().removeClass('active');
				owl&&owl.to(current);
			});
			...
			$('#owl-demo-2 .owl-item').eq(0).addClass('active');
		}, getCurrent: function(){
			var current = owl.current();
			$('#owl-demo-2 .owl-item').eq(current).addClass('active');
			owlSmall&&owlSmall.to(current);
			// window.console&&console.log&&console.log(current);
		}, sticky:function  () {
			var c = "sticky";
			...
			var barHeight 		= $('#menuTop').outerHeight(),
				submenu 		= $('.main-menu ul:visible').length > 0,
				submenuHeight 	= $('.main-menu ul:visible').outerHeight(true),
...
$(window).on('resize', function(){
	...
	historyPan.DetectWidth();
	historyPan.DetectPage();
});
